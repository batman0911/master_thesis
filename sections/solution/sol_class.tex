...
(1983) \cite{}), \textit{deterministic annealing} (Dueck and Scheurer
(1990) \cite{}; Dueck (1993) \cite{}), \textit{variable neighbourhood search} (Mladenović and Hansen (1997) \cite{}), \textit{very large neighbourhood search}(Ergun, Orlin, Steele-Feldman (2006) \cite{}) và \textit{adaptive large neighbourhood search} (Ropke, Pisinger (2006) \cite{}). Các thành phần chính của tìm kiếm lân cận là các quy tắc để xác định vùng lân cận của một nghiệm và cơ chế để khám phá vùng lân cận đó. 

Trong \textit{tabu search} không gian nghiệm được khám phá bằng cách di chuyển từ nghiệm hiện tại đến nghiệm tốt nhất trong một tập con của lân cận của nghiệm đó. Để tránh việc lặp lại các nghiệm, các nghiệm được gán một thuộc tính gắn với nghiệm hiện tại để không được chọn trong một số lần lặp tiếp theo. Một nghiệm trở thành nghiệm tốt nhất trong số các nghiệm đã biết có thuộc tính gắn với thuộc tính hiện tại. Nguyên lý này được trình bày đầu tiên bởi Cordeau, Gendreau, Laporte (1997) \cite{} và hiện nay được biết đến như là phương pháp tìm kiếm dựa trên thuộc tính (Derigs and Kaiser (2007) \cite{}).
